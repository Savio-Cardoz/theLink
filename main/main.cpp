...
// MQTT security: TLS broker verification, HMAC command authenticity, signed OTA
...
// SNTP/clock helpers (defined further down, used from event_handler).
static void sntp_start_once(void);
static bool wait_for_clock(int seconds);
...
// Hash-then-sign: the whole update.bin is hashed with SHA-256 and only
// that digest is signed (ECDSA P-256, DER-encoded). The signature ships
// next to the image as "<file>.sig"; the public key is compiled in from
// firmware_pubkey.h (generated by scripts/sign_update.py --genkey).
// Full process: docs/ota_signing.md.

static bool sha256_file(const std::string &path, unsigned char digest[32])
{
	...
	bool ok = (info != nullptr) && (mbedtls_md_setup(&ctx, info, 0) == 0) &&
	...
	while (ok)
	{
		...
		ok = (mbedtls_md_update(&ctx, buf.data(), n) == 0);
	}
	if (ok)
	{
		ok = (mbedtls_md_finish(&ctx, digest) == 0);
	}
...
static bool verify_firmware_signature(const std::string &filepath)
{
	// 1. SHA-256 of the image (the digest is what was signed).
	unsigned char digest[32];
	if (!sha256_file(filepath, digest))
	{
		ESP_LOGE(TAG, "Firmware verify: cannot hash %s", filepath.c_str());
		return false;
	}

	// 2. Signature file: <image>.sig (DER-encoded ECDSA P-256, ~70 bytes).
	std::string sig_path = filepath + ".sig";
	std::vector<unsigned char> sig(256);
	FILE *sf = fopen(sig_path.c_str(), "rb");
	if (sf == nullptr)
	{
		ESP_LOGE(TAG, "Firmware verify: signature file %s is missing", sig_path.c_str());
		return false;
	}
	size_t sig_len = fread(sig.data(), 1, sig.size(), sf);
	fclose(sf);
	if (sig_len < 64)
	{
		ESP_LOGE(TAG, "Firmware verify: signature file too short (%u bytes)", (unsigned)sig_len);
		return false;
	}

	// 3. Verify against the embedded public key.
	mbedtls_pk_context pk;
	mbedtls_pk_init(&pk);
	int rc = mbedtls_pk_parse_public_key(&pk,
										 (const unsigned char *)THELINK_FIRMWARE_PUBKEY_PEM,
										 strlen(THELINK_FIRMWARE_PUBKEY_PEM) + 1);
	if (rc != 0)
	{
		ESP_LOGE(TAG, "Firmware verify: bad embedded public key (%d)", rc);
		mbedtls_pk_free(&pk);
		return false;
	}
	...
	if (rc != 0)
	{
		ESP_LOGE(TAG, "Firmware verify: signature check failed (mbedtls rc=%d)", rc);
		return false;
	}

	ESP_LOGI(TAG, "Firmware verify: signature OK for %s", filepath.c_str());
	return true;
}
...
static void handle_firmware_update(const std::string &filepath)
{
	...
	// Reject unsigned images (ECDSA P-256 over the SHA-256 of update.bin).
#if defined(CONFIG_OTA_SIGNATURE_VERIFY)
	if (!verify_firmware_signature(filepath))
	{
		ESP_LOGE(TAG, "Firmware update: missing or invalid signature - rejecting image");
		publish_ota_event("failed", "bad firmware signature");
		return;
	}
#else
	ESP_LOGW(TAG, "Firmware update: signature verification is DISABLED (CONFIG_OTA_SIGNATURE_VERIFY=n)");
#endif
						  ...
						  int32_t event_id, void *event_data)
{
	...
	else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
	{
		...
		// TLS certificate validation needs a wall clock; start SNTP now.
		sntp_start_once();
...
// ── System clock (SNTP) ─────────────────────────────────────────────
// TLS broker-certificate verification and the optional MQTT "ts" replay
// window both need a valid wall-clock time.

#ifndef CLOCK_MIN_EPOCH
#define CLOCK_MIN_EPOCH 1577836800LL /* 2020-01-01: sane minimum epoch */
#endif
...
static void sntp_start_once(void)
{
	...
	config.wait_for_sync = false; // Non-blocking; time syncs in the background.
	config.start = true;
	if (esp_netif_sntp_init(&config) != ESP_OK)
	{
		ESP_LOGW(TAG, "SNTP init failed - TLS certificate validation will fail until the clock is set");
	}
	else
	{
		ESP_LOGI(TAG, "SNTP started (clock required for TLS broker verification)");
	}
}

/* Wait up to `seconds` for the system clock to advance past a sane epoch.
 * Returns true once time() looks valid. */
static bool wait_for_clock(int seconds)
{
	time_t deadline = time(nullptr) + seconds;
	while (time(nullptr) < CLOCK_MIN_EPOCH && time(nullptr) < deadline)
	{
		vTaskDelay(pdMS_TO_TICKS(200));
	}
	return time(nullptr) >= CLOCK_MIN_EPOCH;
}
...
// App is fully up: hand the notification LED back to MQTT control.
	...
	case MQTT_EVENT_DATA:
	{
		...
		// ── Command authenticity gate (config-guarded HMAC) ─────────────
		// MQTTX-friendly: does nothing while MQTT_HMAC_MODE is DISABLED.
		// OPTIONAL accepts unsigned commands but drops invalid tags;
		// REQUIRED drops anything that fails mqtt_auth_verify().
		const mqtt_auth_mode_t auth_mode = mqtt_auth_get_mode();
		if (auth_mode != MQTT_AUTH_MODE_DISABLED)
		{
			mqtt_auth_result_t auth = mqtt_auth_verify(event);
			bool allow = (auth == MQTT_AUTH_PASS) ||
						 (auth == MQTT_AUTH_MISSING && auth_mode == MQTT_AUTH_MODE_OPTIONAL);
			if (!allow)
			{
				ESP_LOGW(TAG, "Dropping MQTT command on %.*s: %s",
						 event->topic_len, event->topic, mqtt_auth_result_str(auth));
				free(payload);
				return;
			}
		}
...
static void mqtt5_app_start(void)
{
	...
	// Verify the broker's certificate chain (CA + hostname) for mqtts:// URLs
	// using the ESP-IDF certificate bundle. Without this, TLS would accept any
	// certificate and be open to a man-in-the-middle.
	mqtt5_cfg.broker.verification.crt_bundle_attach = esp_crt_bundle_attach;
	mqtt5_cfg.broker.verification.skip_cert_common_name_check = false;

	// Optional broker credentials (leave CONFIG_MQTT_USERNAME empty for
	// anonymous public brokers).
...
static void wifi_prov_task(void *arg)
{
	...
	// Give the system clock a chance to sync: mqtts:// broker certificate
	// validation (and the optional replay window) needs a valid time. The
	// client also retries on reconnect, so this is only a bounded head start.
	if (!wait_for_clock(10))
	{
		ESP_LOGW(TAG, "System clock not yet synced - TLS may fail until SNTP completes");
	}
	mqtt5_app_start();
