...
{
	s_led_prov_state.store(state);
}

led_prov_state_t prov_get_led_state(void)
{
	return s_led_prov_state.load();
}

// ── Wi-Fi provisioning definitions ──────────────────────────────────
						  ...
						  int32_t event_id, void *event_data)
{
	if (event_base == NETWORK_PROV_EVENT)
	{
		...
		case NETWORK_PROV_START:
			...
			prov_set_led_state(led_prov_state_t::LED_PROV_STATE_PROVISIONING);
			break;
		...
		case WIFI_EVENT_STA_DISCONNECTED:
			{
				...
				ui_notify_network_status(false);
				esp_wifi_connect();
			}
			break;
	...
	else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
	{
		...
		ui_notify_network_status(true);

		/* Signal main application to continue execution */
		xEventGroupSetBits(wifi_event_group, WIFI_CONNECTED_EVENT);
		prov_set_led_state(led_prov_state_t::LED_PROV_STATE_WIFI_CONNECTED);
...
void provisioning_register_core_events(void)
{
	...
	// Hand the notification LED back to MQTT control once the app is up.
	app::register_on_connect([]()
		{
			prov_set_led_state(led_prov_state_t::LED_PROV_STATE_NONE);
		});
}
